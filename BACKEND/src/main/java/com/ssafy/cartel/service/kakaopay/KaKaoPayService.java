...
public class KaKaoPayService {
    ...
    public ReadyResponse payReady() {
        ...
        parameters.add("partner_order_id", payment.getPaymentId());           // 가맹점 주문번호, 최대 100자
        parameters.add("partner_user_id", String.valueOf(user.getId()));          // 가맹점 회원 id, 최대 100자
        parameters.add("item_name", counsel.getTitle());             // 상품명, 최대 100자
        parameters.add("quantity", "1");                        // 상품 수량
        parameters.add("total_amount", String.valueOf(payment.getAmount()));   // 상품 총액
    ...
    public ApproveResponse payApprove(String pg_token) {
        ...
        parameters.add("partner_order_id", payment.getPaymentId());
        parameters.add("partner_user_id", "cartel");
